...
pub fn build(b: *std.Build) void {
    ...
    buildHostTests(b, host_tests_step, optimize);
}
...
/// Host-compatible tests that can be built and run on the development machine
/// Host-compatible tests that can be built and run on the development machine.
/// Note: While test/CMakeLists.txt includes several tests outside PICO_ON_DEVICE,
/// their individual CMakeLists.txt have conditions that skip them on host:
/// - pico_stdio_test: requires pico_multicore (device-only)
/// - pico_time_test: requires alarm support (disabled on host)
/// - pico_divider_test: explicitly checks PICO_ON_DEVICE, uses uart_default
const host_tests: []const []const u8 = &.{
    "pico_stdlib_test",
};

fn generateHostConfigHeaders(b: *std.Build) std.Build.LazyPath {
    const wf = b.addWriteFiles();

    // Generate pico/version.h (same as device builds)
    ...
    // Generate pico_config_platform_headers.h for host platform
    // Host builds don't include a board header
    _ = wf.add("pico_config_platform_headers.h",
        \\// Platform headers for host builds
        \\// No board header needed for host platform
        \\
    );
...
fn buildHostTests(
    b: *std.Build,
    host_tests_step: *std.Build.Step,
    optimize: std.builtin.OptimizeMode,
) void {
    // Generate host config headers
    const host_generated = generateHostConfigHeaders(b);

    for (host_tests) |test_name| {
        ...
        exe.root_module.addCMacro("PICO_NO_HARDWARE", "1");
        exe.root_module.addCMacro("PICO_ON_DEVICE", "0");
        exe.root_module.addCMacro("PICO_BUILD", "1");
        exe.root_module.addCMacro("PICO_TIME_DEFAULT_ALARM_POOL_DISABLED", "1");

        // Generated headers (must come first)
        exe.addIncludePath(host_generated);

        // Host platform include paths (order matters - host headers override common)
        exe.addSystemIncludePath(b.path("src/host/pico_platform/include"));
        exe.addSystemIncludePath(b.path("src/host/pico_runtime/include"));
        exe.addSystemIncludePath(b.path("src/host/hardware_gpio/include"));
        exe.addSystemIncludePath(b.path("src/host/hardware_uart/include"));
        exe.addSystemIncludePath(b.path("src/host/hardware_divider/include"));
        exe.addSystemIncludePath(b.path("src/host/hardware_timer/include"));
        exe.addSystemIncludePath(b.path("src/host/hardware_sync/include"));
        exe.addSystemIncludePath(b.path("src/host/pico_stdio/include"));
        exe.addSystemIncludePath(b.path("src/host/pico_time_adapter/include"));

        // Common headers
        exe.addSystemIncludePath(b.path("src/common/pico_base_headers/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_time/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_sync/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_util/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_bit_ops_headers/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_stdlib_headers/include"));
        exe.addSystemIncludePath(b.path("src/common/pico_divider_headers/include"));

        // Host source files
        const host_sources: []const []const u8 = &.{
            "src/host/pico_platform/platform_base.c",
            "src/host/pico_stdlib/stdlib.c",
            "src/host/pico_stdio/stdio.c",
            "src/host/pico_runtime/runtime.c",
        ...
        // Common source files used by host builds
        exe.addCSourceFile(.{
            .file = b.path("src/common/pico_time/time.c"),
            .flags = &.{"-std=gnu11"},
        });
        exe.addCSourceFile(.{
            .file = b.path("src/common/pico_time/timeout_helper.c"),
            .flags = &.{"-std=gnu11"},
        });
        exe.addCSourceFile(.{
            .file = b.path("src/common/pico_sync/lock_core.c"),
            .flags = &.{"-std=gnu11"},
        });

        // Test source file
        // Use -fwrapv to enable defined wrapping behavior for signed overflow,
        // matching GCC's default behavior that the test code expects
        const test_source = b.fmt("test/{s}/{s}.c", .{ test_name, test_name });
        exe.addCSourceFile(.{
            .file = b.path(test_source),
            .flags = &.{ "-std=gnu11", "-fwrapv" },
        });

        // Install the executable
        const install = b.addInstallArtifact(exe, .{});
        host_tests_step.dependOn(&install.step);

        // Run the test
        const run = b.addRunArtifact(exe);
        run.step.dependOn(&install.step);
        host_tests_step.dependOn(&run.step);
    }
}
